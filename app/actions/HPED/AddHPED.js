...
import {StyleSheet, Alert, Text, Image, View, Navigator, InteractionManager, DrawerLayoutAndroid, StatusBar, TouchableOpacity, TouchableNativeFeedback, DatePickerAndroid, ScrollView, TextInput, Picker, TimePickerAndroid, Slider, Switch, ToastAndroid, Dimensions, Modal, RefreshControl, ListView, AsyncStorage} from 'react-native'
        ...
        if (this.state.chiefComplaint !== '') {
            db.transaction((tx) => {
                ...
                tx.executeSql("SELECT id FROM diagnosis WHERE id BETWEEN "+insertID+" AND "+((insertID*2)-1)+" ORDER BY created_at DESC LIMIT 1", [], (tx, rs) => {
                    ...
                    tx.executeSql("INSERT INTO diagnosis (`id`, `patientID`, `doctorID`, `appointmentID`, `preparedByID`, `chiefComplaint`, `historyIllness`, `bodyTemperature`, `bloodPressure`, `respirationRate`, `pulseRate`, `medicalHistory`, `initialDiagnosis`, `physicalExam`, `services`, `type`, `code`, `category`, `plan`, `pay`, `referringDoctor`, `labs`, `imaging`, `accident`, `painLevel`, `allergies`, `currentMedications`, `date`, `timeStart`, `timeEnd`, `certRemarks`, `certPurpose`, `symptoms`, `deleted_at`, `created_at`, `updated_at`) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", _.values(values), (tx, rs) => {
                        db.insertID = rs.insertId;
                        _.forEach(icds, (v, i) => {
                            var insertID = this.state.mobileID*100000;
